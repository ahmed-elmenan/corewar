...
/*   Updated: 2020/02/07 03:51:01 by ahel-men         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

int	ft_is_j_modifier(char const *format, int *i, t_data *data)
{
	return ((ft_strnstr2(format, "j", 1, *i)) &&
			ft_int_max_detected(data, i));
}

int	ft_is_z_modifier(char const *format, int *i, t_data *data)
{
	return ((ft_strnstr2(format, "z", 1, *i)) &&
			ft_size_t_detected(data, i));
}

int	ft_is_h_modifier(char const *format, int *i, t_data *data)
{
